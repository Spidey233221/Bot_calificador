...
def parse_kumon_exercises(ocr_text, expected_numbers=None):
    ...
    text = re.sub(r'\\\\', '\n', text)  # saltos de linea reales
    text = re.sub(r'[^\d\+\-\=\n]', ' ', text)  # SOLO números y operadores
    text = re.sub(r'\s+', ' ', text)

    # Separar en posibles líneas
    posibles = re.split(r'\n|(?=\d+\s*[\+\-])', text)
    ...
    # 🔥 Si hay expected_numbers, reasignar correctamente
    if expected_numbers:
        ejercicios_reindex = {}
