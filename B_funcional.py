...
PREPROCESS_CONTRAST = 1.6      # Factor de contraste
...
def preprocess_image(input_path, output_path=None):
    """
    Preprocesamiento optimizado para Kumon OCR:
    - Remueve tinta roja sin destruir números
    - Mejora contraste sin deformar dígitos
    - Binarización ligera
    - Upscale controlado
    """
    ...
    # ═══════════════════════════════════════
    # 1. REMOVER ROJO (MENOS AGRESIVO)
    # ═══════════════════════════════════════
    if PREPROCESS_STRIP_RED:
        img = remove_red_ink(
            img,
            red_threshold=150,   # antes 130 → menos agresivo
            dominance=40,        # antes 25 → evita borrar negros
            dilate=3             # antes 5 → no invadir números
        )
    ...
    # ═══════════════════════════════════════
    # 3. CONTRASTE CONTROLADO
    # ═══════════════════════════════════════
    img = ImageOps.autocontrast(img, cutoff=1)
    img = ImageEnhance.Contrast(img).enhance(1.6)  # 🔥 antes 2.5

    # ═══════════════════════════════════════
    # 4. BINARIZACIÓN SUAVE (CLAVE)
    # ═══════════════════════════════════════
    img_np = np.array(img)

    threshold = 160  # puedes probar 150–180
    ...
    # ❌ QUITAMOS median filter (rompe números)

    # ═══════════════════════════════════════
    # 6. UPSCALE CONTROLADO
    # ═══════════════════════════════════════
    if PREPROCESS_UPSCALE and PREPROCESS_UPSCALE != 1:
        new_size = (img.width * PREPROCESS_UPSCALE, img.height * PREPROCESS_UPSCALE)
        img = img.resize(new_size, Image.BICUBIC)  # mejor que LANCZOS para OCR
...
def ocr_image(image_path):
    """Envia imagen a Mathpix (con preprocesamiento + fallback robusto)."""
    
    if not MATHPIX_APP_ID or not MATHPIX_APP_KEY:
        return {"error": "Credenciales Mathpix no configuradas"}

    # ═══════════════════════════════════════
    # 1. INTENTAR CON PREPROCESAMIENTO
    # ═══════════════════════════════════════
    try:
    ...
    try:
        response = send_to_mathpix(path_to_send)
        data = response.json()

        # ═══════════════════════════════════════
        # 🔥 2. FALLBACK SI FALLA (Content not found)
        # ═══════════════════════════════════════
        if "error" in data and "Content not found" in data["error"]:
            print("         ⚠️ Content not found → reintentando SIN preprocesamiento...")

            response = send_to_mathpix(image_path)
            data = response.json()

        # ═══════════════════════════════════════
        # 🔥 3. FALLBACK SI VIENE VACÍO
        # ═══════════════════════════════════════
        if not data.get("text") and not data.get("line_data"):
            print("         ⚠️ OCR vacío → reintentando SIN preprocesamiento...")
...
def split_columns_from_line_data(line_data):
    """
    Divide line_data en columna izquierda y derecha usando coordenada X.
    """
    if not line_data:
        return [], []

    # obtener todos los X
    xs = [line.get("left", 0) for line in line_data if "left" in line]

    if not xs:
        return [], []

    # punto medio dinámico
    ...
    for line in line_data:
        ...
        if x < mid_x:
            izquierda.append((line.get("top", 0), text))
        else:
            derecha.append((line.get("top", 0), text))

    # ordenar por posición vertical
    izquierda.sort(key=lambda x: x[0])
    derecha.sort(key=lambda x: x[0])

    # devolver solo texto
    izquierda = [t for _, t in izquierda]
    derecha = [t for _, t in derecha]
...
def parse_vertical_operations(text):
    """
    Extrae bloques tipo array y los ordena correctamente.
    """

    # 🔥 1. Separar cada bloque array
    bloques = re.findall(r'\\begin\{array\}.*?\\end\{array\}', text, re.DOTALL)
    ...
    for bloque in bloques:
        ...
        limpio = re.sub(r'\\begin\{array\}\{[^}]*\}', '', limpio)
        limpio = re.sub(r'\\end\{array\}', '', limpio)
        limpio = re.sub(r'\\\\', '\n', limpio)
        limpio = re.sub(r'[^\d\+\-\n]', ' ', limpio)
        ...
        if len(lines) >= 3:
            ...
            for i in range(len(lines) - 2):
                ...
                m1 = re.match(r'^(\d+)$', l1)
                m2 = re.match(r'^([\+\-])\s*(\d+)$', l2)
                m3 = re.match(r'^(\d+)$', l3)

                if m1 and m2 and m3:
                    ejercicios.append({
                        "expresion": f"{m1.group(1)} {m2.group(1)} {m2.group(2)}",
                        "resultado_alumno": m3.group(1)
                    })
                    break

    # 🔥 2. ORDENAR (clave)
    # ya vienen en orden natural del OCR → solo enumerar
    resultado = {}
    for i, ej in enumerate(ejercicios):
        resultado[i + 1] = {
            "expresion": ej["expresion"],
            "resultado_alumno": ej["resultado_alumno"],
            "confianza": "alta"
        }
    ...
    while i < len(lines) - 2:
        ...
        if m1 and m2 and m3:
            ...
            ejercicios[index] = {
                "expresion": f"{num1} {op} {num2}",
                "resultado_alumno": res,
                "confianza": "media"
            }
...
def parse_kumon_exercises(ocr_text, expected_numbers=None):
    ...
    # ═══════════════════════════════════════
    # 🔥 1. INTENTAR FORMATO VERTICAL
    # ═══════════════════════════════════════
    vertical = parse_vertical_operations(ocr_text)

    if vertical:
        print(">>> Detectado formato vertical")
        ejercicios = vertical
    else:
        ejercicios = {}

    # ═══════════════════════════════════════
    # 🔥 2. FALLBACK: FORMATO NORMAL (4+4=8)
    # ═══════════════════════════════════════
    if not ejercicios:
        posibles = re.split(r'\n|(?=\d+\s*[\+\-])', text)
        ...
        for linea in posibles:
            ...
            match = re.search(r'(\d+)\s*([\+\-])\s*(\d+)\s*=\s*(\d+)', linea)

            if match:
                ejercicios[index] = {
                    "expresion": f"{match.group(1)} {match.group(2)} {match.group(3)}",
                    "resultado_alumno": match.group(4),
                    "confianza": "alta"
                }
                index += 1

    # ═══════════════════════════════════════
    # 🔥 3. REINDEXAR SI HAY RESPUESTAS ESPERADAS
    # ═══════════════════════════════════════
    if expected_numbers:
        ejercicios_reindex = {}
        for i, num in enumerate(expected_numbers):
            if i < len(ejercicios):
                ejercicios_reindex[num] = ejercicios[i + 1]
            else:
                ejercicios_reindex[num] = {
                    "expresion": "?",
                    "resultado_alumno": None,
                    "confianza": "no_detectado"
                }
        return ejercicios_reindex

    ejercicios = ordenar_ejercicios_por_columnas(ejercicios)
    return ejercicios

def ordenar_ejercicios_por_columnas(ejercicios):
    """
    Reordena ejercicios tipo Kumon:
    primero columna izquierda (top→bottom),
    luego columna derecha (top→bottom)
    """

    items = list(ejercicios.items())

    # extraer números de la expresión para estimar columna
    def get_x_key(e):
        expr = e[1]["expresion"]
        num1 = int(expr.split()[0])
        return num1

    # ordenar por valor base (esto separa columnas naturalmente)
    items.sort(key=lambda x: (get_x_key(x), x[0]))

    # dividir en dos columnas
    mitad = len(items) // 2
    izquierda = items[:mitad]
    derecha = items[mitad:]

    # reordenar final: izquierda completa + derecha completa
    orden_final = izquierda + derecha

    # reconstruir índices
    nuevo = {}
    for i, (_, val) in enumerate(orden_final, 1):
        nuevo[i] = val
...
def process_pair(page, save_dir, level, page_num, do_ocr):
    ...
    if do_ocr and MATHPIX_APP_ID and MATHPIX_APP_KEY:
        ...
        for ss in screenshots:
            ...
            line_data = ocr_result.get("line_data", [])

            # 🔥 fallback si text viene vacío
            if not ocr_text and line_data:
            ...
            # ═══════════════════════════════════════
            # 🔥 SEPARACIÓN POR COLUMNAS (CLAVE)
            # ═══════════════════════════════════════
            if line_data:
                ocr_text = "\n".join([
                    line.get("text", "")
                    for line in sorted(line_data, key=lambda l: (l.get("top", 0), l.get("left", 0)))
                ])

                print(f"\n--- LINE TEXT ORDENADO ({ss['label']}) ---")
                print(ocr_text)
                print("--------------------------------\n")
            else:
                print("         ⚠️ Sin line_data, usando OCR completo")
            ...
            if ejercicios:
                ...
                correctos, total, fuzzy = mostrar_validacion(
                    resultados, f"Validacion {ss['label']}"
                )
